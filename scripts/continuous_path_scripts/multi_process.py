...
def create_env(env_path, worker_id=0, time_scale=2.0, no_graphics=False):
    ...
    unity_env = UnityEnvironment(env_path, side_channels=[channel], worker_id=worker_id, no_graphics=no_graphics, base_port=2500)
...
def train_model(model_name, model_class, worker_id):
    n_envs = 1

    env = r"builds\continuous_path_linux\3DPos.x86_64"
    def make_env_scene(worker_id):
        return lambda: create_env(env, worker_id, time_scale=4.0, no_graphics=True)

    env_fns_scene = [make_env_scene(worker_id) for i in range(n_envs)]
    env_scene = SubprocVecEnv(env_fns_scene)

    base_path = 'continuous_path_models/logs_models'
    trained_models_path = 'continuous_path_models/trained_models'
    os.makedirs(trained_models_path, exist_ok=True)

    save_path_scene = get_save_path(base_path, model_name)
    checkpoint_callback_scene = CheckpointCallback(save_freq=250000, save_path=save_path_scene, name_prefix=model_name)
    ...
    tensorboard_log_path_scene = get_save_path("./logs_graphs", model_name)
    reward_logging_callback_scene = RewardLoggingCallback(log_interval=1000, base_path=base_path, model_name=model_name)

    device = th.device("cuda" if th.cuda.is_available() else "cpu")
    print(f"Using device: {device} for model {model_name}")

    model = model_class("MlpPolicy", env_scene, verbose=2, tensorboard_log=tensorboard_log_path_scene,
                        policy_kwargs=policy_kwargs,
                        device=device)

    model.learn(total_timesteps=2000000, reset_num_timesteps=True, tb_log_name=f"train_{model_name}",
                callback=[checkpoint_callback_scene, reward_logging_callback_scene])
...
if __name__ == '__main__':
    ...
    for model_name, model_class, worker_id in models:
        p = multiprocessing.Process(target=train_model, args=(model_name, model_class, worker_id))
        p.start()
        processes.append(p)
